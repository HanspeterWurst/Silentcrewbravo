...
switch (life_veh_shop select 0) do
{
	...
	case "civ_EPC" : {_ret = license_civ_driver};
	case "civ_nutzfzg" : {_ret =license_civ_nutz && license_civ_truck};
	case "civ_bus" : {_ret = license_civ_bus && lioense_civ_truck};
	...
	case "ISA_car":  {_ret = license_cop_isa;};
	case "cop_ship": 
		{
