...
/*	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent,east]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Truck_01_mover_F"]) then {
    ...
    case 16:
    {
        if ((life_inv_pickaxe >0) && (vehicle player == player) && !life_action_pickaxeInUse) then{
