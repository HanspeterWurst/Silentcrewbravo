...
if(isNull _curTarget) exitWith {
	if(_isWater) then {
		...
		if (playerSide == civilian && !life_action_gathering && life_inv_pickaxe == 1) then 
		{
			[] call life_fnc_pickaxeUse;
		};
		if (playerSide == civilian && !life_action_gathering && life_inv_axe == 1) then 
		{
			[] call life_fnc_axeUse;
		};
		if (playerSide == civilian && !life_action_gathering && life_inv_netz == 1) then 
		{
			[] call life_fnc_netzUse;// Oldie
		};
	};
};
